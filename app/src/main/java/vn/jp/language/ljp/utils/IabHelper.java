...
public class IabHelper {
    ...
    String mDebugTag = "--- billing --- SIAPv3";
...
//                mServiceConn, Context.BIND_AUTO_CREATE);
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK) {
            ...
            Log.i(TAG, "Purchase data: " + purchaseData);
            Log.i(TAG, "Data signature: " + dataSignature);
            Log.i(TAG, "Extras: " + data.getExtras());

            if (purchaseData == null || dataSignature == null) {
                Log.e(TAG, "BUG: either purchaseData or dataSignature is null.");
                Log.e(TAG, "Extras: " + data.getExtras().toString());
            ...
            Log.i(TAG, "Result code was OK but in-app billing response was not OK: " + getResponseDesc(responseCode));
            if (mPurchaseListener != null) {
                result = new IabResult(responseCode, "Problem purchashing item.");
                mPurchaseListener.onIabPurchaseFinished(result, null);
            }
        } else if (resultCode == Activity.RESULT_CANCELED) {
            Log.i(TAG, "Purchase canceled - Response: " + getResponseDesc(responseCode));
            result = new IabResult(IABHELPER_USER_CANCELLED, "User canceled.");
            if (mPurchaseListener != null) {
                mPurchaseListener.onIabPurchaseFinished(result, null);
            }
        } else {
            Log.e(TAG, "Purchase failed. Result code: " + Integer.toString(resultCode)
    ...
    public Inventory queryInventory(boolean querySkuDetails, List<String> moreSkus) throws IabException {
        Log.i(TAG, "queryInventory querySkuDetial:" + querySkuDetails);
    ...
    void consume(Purchase itemInfo) throws IabException {
        Log.i(TAG, "consume " + itemInfo.getSku());
        checkSetupDone("consume");
        try {
            String token = itemInfo.getToken();
            String sku = itemInfo.getSku();
            if (token == null || token.equals("")) {
                Log.e(TAG, "Can't consume " + sku + ". No token.");
                throw new IabException(IABHELPER_MISSING_TOKEN, "PurchaseInfo is missing token for sku: "
                        + sku + " " + itemInfo);
            }

            Log.i(TAG, "Consuming sku: " + sku + ", token: " + token);
            int response = mService.consumePurchase(3, mContext.getPackageName(), token);
            if (response == BILLING_RESPONSE_RESULT_OK) {
                Log.i(TAG, "Successfully consumed sku: " + sku);
            } else {
                Log.i(TAG, "Error consuming consuming sku " + sku + ". " + getResponseDesc(response));
    ...
    void checkSetupDone(String operation) {
        if (!mSetupDone) {
            Log.e(TAG, "Illegal state for operation (" + operation + "): IAB helper is not set up.");
